...
void display(int fx)
{
	...
	if (config.show_info)
		info();
	if (config.show_kml)
		kml_display(screen, x, y, z);
	if (config.show_kml)
		gmapjson_display(screen, x, y, z);
...
#define ENTRY(position, format...) \
	sprintf(temp, format);           \
	print(next, MENU_LEFT, MENU_TOP + position * MENU_Y, temp);
...
void export_cache(int mv)
{
	...
	for (i = 0; i < disk_idx; i++)
	{
		...
		if ((infile = fopen(cachename, "rb")) != NULL)
		{
			...
			fclose(infile);
			if (mv)
			{ // rename must have failed.  Try copy/unlink instead.
				unlink(cachename);
				mv++;
			}
		}
...
void init(int favNum)
{
	...
	s--;
	next_service(); /* decrement, then next_service ensures its good. */
	if (config.cheat)
		DEFAULT_CHEAT_MAP = s;
	else
		DEFAULT_MAP = s;
...
void loop()
{
	...
	while (1)
	{
		...
		if (abs(dx) < JOYSTICK_DEAD)
			dx = 0;
		else
			dx -= abs(dx) / dx * JOYSTICK_DEAD;
		dx *= JOYSTICK_STEP / (32768 - JOYSTICK_DEAD);

		dy = SDL_JoystickGetAxis(joystick, 1);
		if (abs(dy) < JOYSTICK_DEAD)
			dy = 0;
		else
			dy -= abs(dy) / dy * JOYSTICK_DEAD;
